...
import org.expr.mcdm.gui.DomObjects._
...
def button_generate_weights_events(): Unit =
  button_generate_weights.onclick = (e: dom.MouseEvent) =>
    val methodname = select_weight_method.value.toString() match
      ...
      case "3" => {
        problem.weights =
          sd(problem.data, Parser.str2directions(problem.directions)).weights
        "Standard deviation weights"
      }
      case "4" => {
        problem.weights =
          merec(problem.data, Parser.str2directions(problem.directions)).weights
        "Merec weights"
      }
      case "5" => {
        problem.weights =
      ...
      case "8" => {
        // Custom weights
        msgbox(
          "Custom weights are not implemented yet. Assiging equal weights."
        )
        problem.weights =
          Array.fill(problem.criteria.length)(1.0 / problem.criteria.length)
        "Custom weights"
...
def evaluate(problem: MCDMProblem): Unit =
    ...
    if check_moosra.checked then
      val moosraresultLeftRight = moosra(
        problem.data,
        problem.weights,
        Parser.str2directions(problem.directions)
      )
...
@main def hello(): Unit =
  window.console.log("Triggered main entry point. Registering events...")
  register_events()
  window.console.log("Events registered.")
